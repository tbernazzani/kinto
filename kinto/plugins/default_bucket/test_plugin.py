...
# COMP 490 Added by Tim Bernazzani. Test for the existence of the user_id_list, and that any created user ids are in the list
    def test_user_id_of_bucket_is_placed_in_user_id_list(self):
	test_bucket = self.app.get(self.bucket_url, headers=self.headers)
	test_result = bucket.json
	test_settings = self.app.app.registry.settings
	user_id_check = settings['userid_hmac_secret']
	user_id_list_check = self.app.get(user_id_list)

	self.assertIn(user_id_check, user_id_list_check)
#----------------------------------------------------------------------
